...
**Syntax: `step(<step-text>, fn, [done]) { ... }`**

Use the `step()` method to implement your steps. For example:
...
```js
step("Vowels in English language are <vowels>", function (vowels, done) {
  setTimeout(function () {
    done();
  }, 1000);
});
...
```js
step("I am run async and I may fail", function (done) {
    setTimeout(function () {
        try {
...
To have a particular step implementation not break execution, pass an `options` object with a boolean `continueOnFailure` property as the second argument to `step()`. Like this:

```js
step("Vowels in English language are <vowels>.", { continueOnFailure: true}, function (vowelsGiven) {
